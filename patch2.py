...
code = re.sub(r'pub async fn new\([\s\S]*?\.unwrap\(\);', new_fn, code, count=1)
...
code = re.sub(r'pub fn render_egui\([\s\S]*?\{[\s\S]*?\}', cap_new, code) 
# Wait, this regex is wrong, let's not use regex for render_egui.
code = code.replace("pub fn render_egui(\n        &mut self,\n        encoder: &mut wgpu::CommandEncoder,\n        view: &wgpu::TextureView,\n        mut ui_closure: impl FnMut(&egui::Context, &mut Self),\n    ) {\n        if self.egui_state.is_none() { return; }\n        if self.egui_state.is_none() { return; }", "pub fn render_egui(\n        &mut self,\n        encoder: &mut wgpu::CommandEncoder,\n        view: &wgpu::TextureView,\n        mut ui_closure: impl FnMut(&egui::Context, &mut Self),\n    ) {\n        if self.egui_state.is_none() { return; }")

with open("/tmp/patched2.rs", "w") as f:
    f.write(code)
